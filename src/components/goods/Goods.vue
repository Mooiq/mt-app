<template>
  <div class="goods">
	...
	<div class="menu-wrapper" ref="menuScroll">
		<ul>
			<!-- 专场 -->
			<li class="menu-item" :class="{'current':currentIndex === 0}" @click="RscrollY(0)">
				<p class="text">
			...
			<li class="menu-item" v-for="(item,index) in goods" :key="index" :class="{'current':currentIndex === index+1}" @click="RscrollY(index+1)">
				<p class="text">
  					...
  					<li class="food-item" v-for="(food,index) in item.spus" :key=index>
  						...
  						<div class="content">
  							...
  							<div class="extra">
  								...
  								<span class="praise">{{food.praise}}</span>
  							</div>
...
export default {
  ...
  methods:{
	  	...
	  	// 点击左侧，右侧滑
	  	RscrollY(x){
	  		let y = -this.listHeight[x];
	  		this.foodScroll.scrollTo(0,y,600)
	  	},
	  	calculateCount(spus){
	  		let count = 0
	  		spus.forEach((food)=>{
